 ...
 * ring/rod/light are all positioned against that, so this must stay the
 * icon row's height even though the bar's actual rendered height is taller,
 * see `BAR_TOTAL_HEIGHT`.
 */
...
/** The bar's full rendered height — icon row plus the label underneath it. */
const BAR_TOTAL_HEIGHT = TAB_BAR_HEIGHT + LABEL_AREA_HEIGHT;

/** Bottom padding a screen needs so its content clears the floating bar. */
export const TAB_BAR_CLEARANCE = BAR_TOTAL_HEIGHT + 24;
...
const LABEL_GAP = 4;
/** The bar's border. Load-bearing for coordinates — see `centerOf`. */
...
function GlassArcBar({
  ...
  /**
   * Center of column `i`, in the bar's PADDING-box coordinates — the space
   * the columns and the overlay both lay out in, i.e. inside the 1px border.
   */
  const centerOf = useCallback(
    (index: number) => {
      "worklet";
      const inner = barWidth.value - BORDER_WIDTH * 2;
      if (inner <= 0 || columnCount === 0) return 0;
      return (inner * (index + 0.5)) / columnCount;
    },
  ...
  const nearestIndexTo = useCallback(
    (x: number) => {
      ...
      const raw = (x / inner) * columnCount - 0.5;
  ...
  const pan = Gesture.Pan()
    ...
    .onUpdate((event) => {
      ...
      // gesture is attached to — the bar's BORDER box — so shift it into the
      // padding-box space `centerOf` works in. Absolute position rather than
      // `translationX` added to a captured origin: no anchor to go stale.
      const x = event.x - BORDER_WIDTH;
      head.value = clamp(x, centerOf(0), centerOf(columnCount - 1));
  ...
  const handleBarLayout = useCallback(
    (event: LayoutChangeEvent) => {
      ...
      if (!settling.value && !dragging.value && activeIndex >= 0) {
        const inner = width - BORDER_WIDTH * 2;
        const cx = columnCount > 0 ? (inner * (activeIndex + 0.5)) / columnCount : 0;
        head.value = cx;
        tail.value = cx;
      }
    },
    [activeIndex, barWidth, columnCount, dragging, head, settling, tail],
  );
      ...
      <GestureDetector gesture={pan}>
        <View
          ...
          style={[
            {
              flexDirection: "row",
              // `flex-start`, not `center`: each column needs to start at
              // the bar's own top edge so its icon lands in the first
              // `TAB_BAR_HEIGHT` px — exactly where `rowCenterY` and the
              // ring/rod/light overlay expect it — with the label
              // occupying the extra `LABEL_AREA_HEIGHT` below that.
            ...
            return (
              ...
              >
                {/* Fixed to exactly the icon row's height so the icon's true
                    center lands at `rowCenterY` regardless of the label
                    below it — the ring/rod/light overlay is positioned
                    against that same constant and knows nothing about this
                    column's actual (taller) height. */}
                <View
                  style={{
