...
- **Reuse existing primitives before writing new ones.** Check `mobile/src/components/ui/` (`Button`, `TextField`, `Chip`, `FieldGroup`, `ActionSheet`, `PasswordInput`, `Skeleton`, …) and `mobile/src/features/<feature>/` for an existing shell (e.g. `features/auth/AuthLayout.tsx`, `features/setup/SetupLayout.tsx`) before building a new one. Extend or compose what's there; don't fork a parallel version of a button or input.
- **`components/screens/` is template scaffolding only** — the interchangeable `TabDesignN`/`OnboardingDesignN`-style design variants the project started from. Never put real feature code there. A real screen and its feature-specific sub-components (a layout shell, a card, anything not generic enough for `components/ui/`) go in `features/<feature-name>/` instead — see `features/auth/`, `features/setup/`, `features/home/` for the pattern.
